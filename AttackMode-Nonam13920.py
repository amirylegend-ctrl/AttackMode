        ...
        try:
            ...
            if remaining_days < 0:
                ...
                if remaining_days > 30:
                    status = f"{remaining_days} روز باقی‌مانده"
                    status_color = (0, 1, 0)
...
class AttackSettingsPanel:
    ...
    def _save(self):
        new_commands = []
        new_delays = []
        for i, (_, cmd_entry, delay_entry, _) in enumerate(self.rows):
...
def find_client_id_by_name(name):
    """پیدا کردن client_id اولین بازیکن با نام مشخص (تطابق nickname یا account name)"""
    try:
        roster = get_game_roster()
        for client in roster:
            if 'players' in client and client['players']:
...
def send_next_attack(target_id):
    ...
    instance = _singlcmd_instance  
    if instance:
        instance.safe_send_message(response, 'AttackMode')
...
def show_expiry_notification():
    """نمایش نوتیفیکیشن انقضا"""
    push("⛔ تاریخ انقضا شما به پایان رسید", color=(1, 0, 0))
    push(f"تاریخ انقضا: {EXPIRY_DATE}", color=(1, 0.5, 0))
    push("برای دریافت نسخه جدید تماس بگیرید", color=(0.8, 0.8, 0.8))
    gs('error').play()

def show_unauthorized_notification(current_account):
    """نمایش نوتیفیکیشن اکانت غیرمجاز"""
    push("🚫 اکانت غیرمجاز", color=(1, 0, 0))
...
class SinglCMD(Plugin):
    def __init__(s):
        global _singlcmd_instance
        _singlcmd_instance = s         
        ...
        def show_expiry_warning():
            try:
                ...
                if remaining <= 1:
                    if remaining > 1:
                        push(f"⚠️ پلاگین تا {remaining} روز دیگر منقضی می‌شود", color=(1, 0.5, 0))
                    elif remaining == 1:
                        push(f"⚠️ پلاگین تا فردا منقضی می‌شود", color=(1, 0, 0))
                    else:
                        push(f"⚠️ پلاگین امروز منقضی می‌شود", color=(1, 0, 0))
            except:
                pass
